...
class KafkaConsumer:
    ...
    def _consume(self):
        ...
        try:
            ...
            logger.error(f'Could not poll the message --> {self.topic_name_pattern}')
            return 0
        
        if message is None:
            logger.debug('Message is Not Present')
            return 0
        elif message.error() is not None:
            logger.debug('Message Cannot not be consumed due to: {message.error()}')
            return 0
        else:
